...
# Set at module level, not inside the __name__ == "__main__" guard below.
#
# The cross-language harness does not run this file as a script: _python_driver.py compiles
# it and execs it into a module named __cross_lang_workload__, so __name__ is never
# "__main__" and anything in that guard is dead code under the harness. That went unnoticed
# while fork was CPython's Linux default; Python 3.14 changed the default to forkserver,
# which re-imports per worker and cannot inherit the 2.7MB corpus copy-on-write, so it has
# to pickle it to all 16 workers. That measures process startup rather than word counting,
# and it left CPython and PyPy using different primitives, which the comparison assumes they
# do not.
